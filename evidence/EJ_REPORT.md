...
> incluía `chat_agente_family` (chat_agente, chat_agente_sia,
> chat_agente_sia_v2) como uno de los 3 bloques de dominio centrales del
> análisis. Se confirmó que esas 3 condiciones tienen 34-44% de cada
> respuesta como markup HTML idéntico repetido (`CHAT_AGENTE_MARKUP_CONFOUND_REPORT.md`)
...
**Fecha:** 2026-08-28 (recalculado, versión sin chat_agente)
...
automata_neutro.
**Resultados:** `LSGOT_v4/scripts/fase0/EJ_sin_chat_agente_results.json`

## 1. Resumen

Sin chat_agente-family, la RDM tiene una estructura limpia e
...
hallazgo de `CORRECCION_DVHAT_SIN_CHAT_AGENTE.md`: restricción tiene una
...
- **Condiciones:** axis_family = {axis, axis_short, axis_pec_only},
  ...
  "restricción pura" que sobrevive tras excluir chat_agente-family).
...
- **Confound de dominio (chat_agente):** resuelto por exclusión completa,
  no por control estadístico — ver `CHAT_AGENTE_MARKUP_CONFOUND_REPORT.md`.
...
`CORRECCION_DVHAT_SIN_CHAT_AGENTE.md` (v̂ projection: automata_neutro
...
se ve incluso más nítido de lo que mostraba la versión con chat_agente.**
...
de este análisis (con chat_agente) sugería la jerarquía "dominio >
restricción > identidad" — con chat_agente removida, la jerarquía
correcta es simplemente **"restricción > identidad"**, sin el término de
dominio (que era, en retrospectiva, el propio confound de chat_agente).
