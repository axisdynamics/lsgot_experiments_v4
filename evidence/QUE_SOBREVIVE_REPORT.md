...
|---|---|---|
| `chileatiende` / `chileatiende_sia` / `chileatiende_sia_v2` como condiciones de restricción/wiring | Wrapper HTML repetido en el 100% de las respuestas — confound de formato leído como señal geométrica | `automata_neutro` (0% markup, misma densidad de restricción) sostiene el Factor 1 por sí solo |
| Prediction 2(ii) — "declarado-pero-no-cableado recupera peor" | Dependía enteramente de `chileatiende_sia`/`_v2`, ambas excluidas | Marcada explícitamente como **no testeada**, no refutada — ver `paper/lsgot_4.md` §3.3, §5, §7.2 |
| Jerarquía original "dominio > restricción > identidad" (E-J, versión con chileatiende) | El término "dominio" era el propio confound de chileatiende | Jerarquía revisada, sin ese término espurio (`EJ_REPORT.md`) |
