...
We have since run the same audit on the present Gemma-4-31B-it panel (`CURVATURE_SELF_AUDIT_REPORT.md`), and the failure mode replicates in part. A split-half noise floor for W₁ (B = 1000) shows `automata_neutro`'s two comparisons well above it (×3.5–17 the split-half median) but `axis_pec_only` vs `vanilla` — the identity-only contrast — squarely inside it (0.4–0.5× the median): the statistic has real signal, but only for constraint density, never for identity alone, consistent with every other metric in this paper. Simple baselines beat it in all 4/4 tested comparisons (centroid distance, MMD, and a linear probe on v1 with AUC = 1.000, all p ≤ 0.002 — including the pair where Δκ itself sits in the noise floor). A lighter, proxy-level version of the first-token check (lexical first-word contingency and an OLS residualization of ‖v1‖ on condition plus first-word dummies, not the forward-pass-with-forced-token control the sibling audit used) does *not* reverse any of this panel's own ‖v1‖-based effects the way it did on the sibling panel — they survive and in one case roughly double after the control — though we have not yet run the definitive forced-token version, so this is reassuring rather than conclusive. None of this changes how the statistic is used in this paper: it was already excluded from every primary claim, and the audit confirms rather than overturns that decision.
...
**n = 1 per condition at the level of the system-prompt manipulation.** Each of the seven conditions is a single system-prompt instantiation; twenty *content* prompts are tested within each, but the manipulation itself (the specific wording of, e.g., `automata_neutro`) is not replicated. `axis_pec_only` and `automata_neutro`, the two cells carrying the central argument, are priority targets for replication before this design's conclusions should be read as more than a strong, internally consistent single case.
